...
class VRControlNode:
    ...
    def hand_callback(self, data):
        ...
        self.callback_counter %= 1000

        # 获取当前时间和位置
        hand_position = np.array(data.data[0:3])  # 只取xyz坐标

        ################## safety check 1 ###################
        if all(hand_position < 0.01):
            print("hand out of detection ", self.callback_counter)
            return
        ###################################################
        ...
        ################## safety check 2 ###################
        hand_position[0] = np.clip(hand_position[0], 0.25, 0.8)  # 防止x轴位置过大
        hand_position[1] = np.clip(hand_position[1], -0.7, 0.7)  # 防止y轴位置过大
        hand_position[2] = np.clip(hand_position[2], 0.12, 0.8)  # 防止z轴位置过低
        ###################################################
        ...
        ################## safety check 3 ###################
        if any(dq > 1.0):
            print("dq too large, invaid, ignored", self.callback_counter)
            return
        ###################################################

        ################## safety check 4 ###################
        velocity_threshold = 0.5
        if any(dq < -velocity_threshold) or any(dq > velocity_threshold):
            dq = np.clip(dq, -velocity_threshold,
                         velocity_threshold)  # 防止关节速度过大
            print("dq clipped to threshold", self.callback_counter)
        #####################################################
        ...
        # dq_temp = self.current_q + dq.flatten() * 0.5
        dq_temp = dq.flatten() * 0.5

        joint_command_msg = Float64MultiArray()
        joint_command_msg.data = dq_temp.tolist()
